...
system auto-detects Class + Section columns, converts I→1 through
XII→12, groups rows by unique class/section combinations, and
...
errors are silently ignored if the column already exists. A `migrateDb()`
...
Regular student export uses the same pipeline for consistency.
...
indigo column headers, alternating row fills, gold/silver/bronze rank
highlights.
...
auto-generated a `reason` string ("Weighted average 36% above threshold 33%")
for every auto-promoted student. The Exceptional detection logic checked for
any `reason` field — making every auto-promoted student appear Exceptional.
The fix removed auto-generated reasons; `reason` is now only set manually by
the teacher for genuine exceptions.

## Known Limitations

- **Single-user, single-machine:** One password per school. No multi-teacher
  login or concurrent access. The school computer is the single point of use.
- **No built-in transfer certificates:** Government schools need physical
  certificates with official stamps and seals. The app tracks student status
  and results but does not generate official documents.
- **No timetable or fee management:** By design — these features require
  school-specific policy rules that vary too widely to standardise.
- **No cloud sync or remote backup:** Backup is manual — copy the `data.db`
  file. This is intentional (no internet requirement) but means the school
  is responsible for their own backup discipline.
- **ExcelJS adds ~940KB gzipped to bundle size:** The styling engine is large
  but the trade-off is accepted because professional export formatting is a
  core requirement for government school records.
...
- Hindi voice input for attendance marking
...
own student form — adding "Caste Category" or "Bus Route" with no code
changes.
...
ExcelJS's cell-level API (fill, font, border, alignment, merge) works
consistently and produces output that school administrators recognise as
"official-looking".

**IPC security must be by design, not by hope.** Named handlers
(`students:create`, `attendance:mark`) rather than a generic `db:query`
endpoint mean the renderer cannot execute arbitrary SQL. This was a
decision made in the first hour of architecture that paid off every
subsequent day.
...
and `log('RENDERER: ' + msg)` to every catch block made debugging
tractable.
