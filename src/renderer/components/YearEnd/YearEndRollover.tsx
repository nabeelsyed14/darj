...
export default function YearEndRollover() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      // Load all students for status management
      ...
      const summary = await Promise.all(withSections.map(async (cls: any) => {
        ...
        for (const sec of cls.sections) {
          ...
          total += students.filter((s: any) => s.status === 'active').length
  ...
  // Compute next academic year automatically
  const getNextYear = () => {
    if (!school?.academic_year) return ''
    const parts = school.academic_year.split('-')
    if (parts.length === 2) {
  ...
  const handleArchive = async () => {
    ...
    if (!nextYear) { message.error('Could not calculate next academic year'); return }
    setArchiving(true)
    try {
      await window.api.archives.archive(school.id, school.academic_year)
        ...
        for (const sec of secs) {
          ...
          const studentData = await Promise.all(students.map(async (s: any) => {
            ...
            for (const [, d] of examMap) {
              ...
              wT += avg * d.weight; tW += d.weight
            }
            const pct = tW > 0 ? Math.round(wT / tW) : 0
            const promos = await window.api.promotions.getByStudent(s.id)
            const promo = promos.find((p: any) => p.academic_year === school.academic_year)
            let result = 'Active'
            ...
            return { student: s, pct, result, rawResult: result }
          }))
          ...
          summaryData.push({
            class: `${cls.name} ${sec.name}`, total: students.length,
      ...
      if (summaryData.length > 0) {
        ...
        summaryData.forEach((d, i) => {
          const r = i + 2; sws.getCell(r, 1).value = d.class; sws.getCell(r, 2).value = d.total
          sws.getCell(r, 3).value = d.topperName; sws.getCell(r, 4).value = d.topperPct
        })
      ...
      for (const { studentData, sheetName } of allSheetData) {
        ...
        r1.font = { name: 'Calibri', bold: true, size: 18, color: { argb: 'FF8B0000' } }; r1.alignment = { horizontal: 'center', vertical: 'middle' }; ws.getRow(1).height = 35
        ws.mergeCells(2, 1, 2, cols)
        const r2 = ws.getCell('A2'); r2.value = [school.address, school.principal_name].filter(Boolean).join(' | ')
        r2.font = { name: 'Calibri', size: 11, color: { argb: 'FF555555' } }; r2.alignment = { horizontal: 'center' }
        ws.mergeCells(4, 1, 4, cols)
        const r4 = ws.getCell('A4'); r4.value = `Year End Report: ${school.academic_year}`
        r4.font = { name: 'Calibri', bold: true, size: 13, color: { argb: 'FF1E3A5F' } }; r4.alignment = { horizontal: 'center' }
        ws.mergeCells(5, 1, 5, cols)
        const r5 = ws.getCell('A5'); r5.value = `Generated: ${dayjs().format('DD/MM/YYYY')}`
        r5.font = { name: 'Calibri', size: 10, color: { argb: 'FF555555' } }; r5.alignment = { horizontal: 'center' }

        ;['Full Name', 'Result', 'Percentage', 'Class Rank'].forEach((h, i) => { ws.getCell(7, i + 1).value = h })
        styleHeader(ws, 7, cols)

        for (let i = 0; i < studentData.length; i++) {
          const d = studentData[i]; const row = 8 + i
          ws.getCell(row, 1).value = d.student.field_values?.full_name || d.student.student_uid
          ws.getCell(row, 2).value = d.rawResult; ws.getCell(row, 3).value = d.pct > 0 ? `${d.pct}%` : '—'
        ...
        const fRow = 8 + studentData.length; ws.mergeCells(fRow, 1, fRow, cols)
        const fCell = ws.getCell(fRow, 1); fCell.value = `${studentData.length} students | Generated by Darj | ${now}`
        fCell.font = { name: 'Calibri', italic: true, size: 9, color: { argb: 'FF888888' } }
        ws.columns = [{ width: 25 }, { width: 14 }, { width: 14 }, { width: 12 }]
      }

      const dateStr = new Date().toISOString().split('T')[0]
      const filename = `Darj_YearEnd_${school.academic_year}_${dateStr}.xlsx`
      ...
      message.success(`Rollover complete. ${school.academic_year} has been archived. Report saved as ${filename}`)
      setShowRollover(false); loadAll()
    } catch (e: any) { message.error(`Rollover failed: ${e?.message || t('errors.generic')}`) }
  ...
  const handleChangeStatus = async () => {
    ...
    try {
      ...
      setShowStatusModal(false); setStatusReason('')
      loadAll()
  ...
  return (
    <div>
      ...
      <Alert type="info" showIcon icon={<InfoCircleOutlined />} message="About Year-End Rollover"
      ...
      <Card title="Manage Student Status" style={{ marginBottom: 24, borderTop: '3px solid #6366F1' }}>
        <Table dataSource={allStudents.filter((s: any) => s.status === 'active')} columns={statusColumns} rowKey="id" loading={loading} pagination={{ pageSize: 10 }} size="small" />
      </Card>

      <Card title="Roll Over to New Academic Year" style={{ marginBottom: 24, borderTop: '3px solid #F97316' }}>
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          <Button type="primary" style={{ background: '#F97316', borderColor: '#F97316', marginTop: 12 }} onClick={() => setShowRollover(true)} loading={archiving}>
            Archive & Roll Over
          </Button>
        </Space>
      </Card>

      <Modal title="Confirm Rollover" open={showRollover} onOk={handleArchive} onCancel={() => setShowRollover(false)} confirmLoading={archiving} okText="Confirm Rollover">
        <Space direction="vertical">
          <Text>This will close <strong>{school?.academic_year}</strong> and begin <strong>{nextYear}</strong>.</Text>
          <Text>All promoted students will move to the next class. Year end report will be saved to Desktop.</Text>
          <Text type="secondary">Are you sure?</Text>
        </Space>
      </Modal>
      ...
      <Modal title="Change Student Status" open={showStatusModal} onCancel={() => setShowStatusModal(false)} onOk={handleChangeStatus} okText="Save">
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          <Select value={newStatus} onChange={setNewStatus} style={{ width: '100%' }}
            options={[{ label: 'Active', value: 'active' }, { label: 'Withdrawn', value: 'withdrawn' }, { label: 'On Leave', value: 'on_leave' }, { label: 'Transferred', value: 'transferred' }]} />
